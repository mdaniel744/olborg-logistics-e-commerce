    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-[#E0E2E5]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 md:h-20">
          <Link to={pathFor("home", lang)} className="flex items-center gap-2.5 shrink-0" aria-label="Olborg Logistics">
            ...
            <span className="font-heading font-bold tracking-tight text-[#1A1C1E] text-lg leading-none">
              OLBORG<span className="block text-[10px] font-mono font-medium tracking-[0.25em] text-[#6B7075]">LOGISTICS</span>
            </span>
          </Link>

          <nav className="hidden lg:flex items-center gap-7" aria-label="Main">
            {navItems.map((item) => (
              <Link key={item.to} to={item.to} className="text-sm font-medium text-[#3A3E42] hover:text-[#E65100] transition-colors">
                {item.label}
              </Link>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <button
              ...
              className="relative p-2 text-[#1A1C1E] hover:text-[#E65100] transition-colors"
              aria-label={t("nav.cart")}
            >
              <ShoppingCart className="w-5 h-5" />
              {count > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-[#E65100] text-white text-[10px] font-mono font-bold w-4.5 h-4.5 min-w-[18px] min-h-[18px] flex items-center justify-center">
                  {count}
                </span>
              )}
            </button>
            <Button asChild className="hidden md:inline-flex bg-[#E65100] hover:bg-[#C74600] text-white rounded-none font-semibold">
      ...
      {mobileOpen && (
        <nav className="lg:hidden border-t border-[#E0E2E5] bg-white" aria-label="Mobile">
          <div className="px-4 py-3 space-y-1">
            ...
            <Button asChild className="w-full bg-[#E65100] hover:bg-[#C74600] rounded-none font-semibold mt-2">
