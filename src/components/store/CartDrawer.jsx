...
export default function CartDrawer() {
  ...
  const gross = pricesKnown ? round2(netTotal * (1 + rate / 100)) : null;
          ...
          <>
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              {items.map((item) => {
                ...
                return (
                  <div key={itemKey} className="flex gap-3 border border-[#E0E2E5] bg-white p-3">
                    {item.image && (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center border border-[#E0E2E5]">
                          <button className="p-1.5" onClick={() => updateQuantity(itemKey, item.quantity - 1)} aria-label="-">
                            <Minus className="w-3.5 h-3.5" />
                          </button>
                          <span className="font-mono text-sm w-8 text-center">{item.quantity}</span>
                          <button className="p-1.5 disabled:opacity-40" onClick={() => updateQuantity(itemKey, item.quantity + 1)} disabled={item.quantity >= 100} aria-label="+">
                            <Plus className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <span className="font-mono text-sm font-semibold">
                          {formatMoney(Number.isFinite(unitNet(item)) ? round2(unitNet(item) * item.quantity * (1 + rate / 100)) : null, currency)}
                        </span>
                      </div>
                    </div>
            ...
            <div className="border-t border-[#E0E2E5] px-5 py-4 space-y-3 bg-white">
              ...
              {!pricesKnown && <p className="text-sm text-red-600">{t("checkout.priceUnavailable")}</p>}
