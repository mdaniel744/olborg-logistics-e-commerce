...
export default function ProductDetail() {
  ...
  const selectCondition = (c) => {
    setCondition(c);
    setSearchParams({ [paramKey]: CONDITION_VALUES[lang][c] }, { replace: true });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="lg:sticky lg:top-24 self-start">
          <div className="aspect-[4/3] bg-[#E0E2E5] border border-[#E0E2E5] overflow-hidden">
            <Image src={gallery[0]} alt={lang === "de" ? product.name_de : product.name_pl} className="w-full h-full object-cover" />
          </div>
          {gallery.length > 1 && (
        ...
        <div>
          ...
          <p className="font-mono text-xs text-[#6B7075] mt-2">
            {t("common.sku")}: {variant?.sku || "—"} · {product.size} · {t(`common.${product.container_type}`)}
          </p>
          <p className="mt-4 text-[#3A3E42] leading-relaxed">
            {lang === "de" ? product.short_description_de : product.short_description_pl}
          </p>
          ...
          <div className="mt-6 border-y border-[#E0E2E5] py-5">
            {price ? (
              <>
                ...
                <p className="font-mono text-xs text-[#6B7075] mt-0.5">{t("product.deliveryNotIncluded")}</p>
              </>
            ) : (
          ...
          {/* Variant selectors */}
          <div className="mt-6">
            <p className="font-mono text-xs tracking-widest uppercase text-[#6B7075] mb-2">{t("common.condition")}</p>
          ...
          <div className="mt-6 flex gap-3 items-end flex-wrap">
            <div>
              <p className="font-mono text-xs tracking-widest uppercase text-[#6B7075] mb-2">{t("common.quantity")}</p>
          ...
          <p className="mt-3 text-xs text-[#6B7075] flex items-center gap-1.5">
